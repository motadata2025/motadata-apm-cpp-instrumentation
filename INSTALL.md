...
| **What you install** | The Instrumentation C++ SDK (build tools only — stays on your machine) |
...
*   **What is a Trace?** A trace is a complete record of one request or operation flowing through your application.
*   **What is a Span?** A span is one individual unit of work inside a trace. You create a span by calling `startSpan("functionName")` at the beginning and `span->end()` at the end.
*   **What is CMake?** The build system used to compile C++ projects.
*   **What is a static library (.a file)?** When you compile the library, the output is `libmotadata.a`, which gets embedded into your application.
*   **Why must the Instrumentation SDK be built on the client machine?** C++ libraries are machine-specific. Building it on your machine ensures compatibility with your OS and compiler.

---

## PHASE 1: Environment Setup
### Install build tools and compile the Instrumentation C++ SDK
...
#### 2. Download the Instrumentation C++ SDK Source Code
```bash
cd /opt
...
#### 3. Compile and Install the Instrumentation C++ SDK (Takes 15-30 min)
```bash
...
├── src/
│   └── main.cpp
└── motadata-cpp/                   ← The cloned instrumentation library
    ├── include/
    │   └── motadata.h
    ├── src/
    │   └── motadata.cpp
